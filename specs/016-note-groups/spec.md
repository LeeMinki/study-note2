...
로그인한 사용자는 자신의 노트를 정리하기 위한 그룹을 만들고, 그룹 이름을 나중에 수정하거나 더 이상 필요 없는 그룹을 삭제할 수 있다. 그룹은 태그보다 더 큰 분류 단위로 사용되며, 사용자 본인의 계정 안에서만 보이고 관리된다.
...
1. **Given** 로그인한 사용자가 그룹 관리 영역을 보고 있을 때, **When** 새 그룹 이름을 입력해 저장하면, **Then** 해당 그룹이 즉시 그룹 목록에 표시된다.
2. **Given** 사용자가 본인이 만든 그룹을 보고 있을 때, **When** 그룹 이름을 변경하면, **Then** 변경된 이름이 그룹 목록과 노트 작성/수정 화면의 선택 항목에 반영된다.
3. **Given** 사용자가 본인이 만든 그룹을 보고 있을 때, **When** 그룹을 삭제하면, **Then** 해당 그룹은 목록에서 사라지고 그 그룹에 속했던 노트는 그룹 없음 상태가 된다.
...
로그인한 사용자는 그룹을 선택해 해당 그룹에 속한 노트만 볼 수 있다. 그룹 필터는 기존 검색과 태그 필터와 함께 사용할 수 있어야 하며, 그룹 없음 노트도 별도 기준으로 찾을 수 있다.
...
2. **Given** 그룹 필터가 적용된 상태에서, **When** 사용자가 검색어를 입력하거나 태그를 선택하면, **Then** 그룹 조건과 검색/태그 조건을 모두 만족하는 노트만 표시된다.
...
- 같은 계정 안에서 같은 이름의 그룹을 중복 생성하려는 경우 중복을 방지해야 한다.
...
- **FR-012**: The system MUST apply group filtering together with existing search and tag filters.
...
- **FR-016**: The system MUST validate group names by trimming surrounding whitespace, rejecting empty values, rejecting duplicate names within the same account, and enforcing a reasonable maximum length.
...
- **CA-004**: Group creation and renaming should prefer inline or page-embedded interactions consistent with the current app; modals are not part of the expected MVP interaction.
...
- **Group**: A user-owned structural category for notes. Key attributes include a display name, owner, creation time, and update time.
...
- **SC-004**: Search, tag filtering, and group filtering can be combined without losing any active filter state.
...
- A group name is unique only within one user's account; different users may use the same group name.
...
- Group ordering defaults to a predictable user-friendly order, such as most recently updated or alphabetic, to be finalized during planning without changing feature scope.
