...
본문은 Markdown 문법으로 작성할 수 있어야 하며, 작성 이후 같은 화면에서 바로 인라인으로 수정할 수 있어야 한다.
...
사용자는 최근에 작성한 노트를 빠르게 훑어보고 언제 작성했는지 즉시 이해할 수 있어야 한다.
...
- **FR-001**: System MUST allow a user to create a note with `title`, `content`, `tags`, `createdAt`,
  and `updatedAt`.
- **FR-002**: System MUST allow note content to be written using Markdown syntax.
- **FR-003**: System MUST allow users to edit an existing note inline within the note view, not through a modal.
...
- **FR-011**: System MUST update visible results immediately when search text or tag filters change.
...
- **CA-004**: The planned editing model is inline-first because the constitution explicitly prefers inline
  editing over modals for fast, keyboard-friendly note updates.
