...
1. **Given** 개발자가 PR을 생성한 상태, **When** 자동 품질 체크가 실행되면, **Then** lint, format, build, test의 핵심 결과가 PR에서 확인 가능해야 한다.
...
- **FR-001**: System MUST define a minimum frontend test strategy that covers the most regression-prone user-visible behavior.
- **FR-002**: System MUST define a minimum backend test strategy that covers authentication, protected actions, JSON response envelopes, and local JSON persistence behavior.
- **FR-003**: System MUST identify which test types are introduced first and which advanced test types remain outside the MVP scope.
- **FR-004**: System MUST define a quality-check structure for lint, format, build, and test, including which checks are required for PR review.
- **FR-005**: System MUST run the selected core quality checks automatically for GitHub PRs before merge.
- **FR-006**: System MUST keep PR validation separate from production deployment; PR checks must not publish images, mutate GitOps state, or deploy to production.
- **FR-007**: System MUST define required status check names that can be connected to GitHub branch protection.
- **FR-008**: System MUST document how developers run the same quality checks locally in the primary Linux/WSL development environment.
- **FR-009**: System MUST document failure interpretation criteria so developers can distinguish lint, format, build, test, manifest, and environment failures.
- **FR-010**: System MUST introduce at least one meaningful automated test where it reduces current regression risk, rather than only adding build-only checks.
- **FR-011**: System MUST avoid strong coverage enforcement in the MVP quality gate.
- **FR-012**: System MUST keep E2E testing, performance testing, advanced security scanning, and strict coverage policy outside this MVP scope.
- **FR-013**: System MUST avoid installing or adding new project dependencies until the user explicitly approves them.
- **FR-014**: System SHOULD keep the first quality gate simple enough to be maintained by one developer and understood during PR review.
- **FR-015**: System SHOULD leave extension points for future broader E2E, accessibility, security, coverage, and production smoke checks.
...
- **In Scope**: Minimum frontend and backend test strategy, first test type selection, lint/format/build/test check structure, GitHub PR quality checks, required status check definitions, local execution documentation, failure triage documentation.
