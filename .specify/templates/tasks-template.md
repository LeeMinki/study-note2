...
- For Study Note, default to the web app layout with `backend/` and `frontend/`
...
- [ ] T003 Record any new package requirement and request user approval before installation
...
- [ ] T010 Define or update shared API contract notes for `{ success, data, error }`
...
- [ ] T018 [US1] Verify inline editing / keyboard interaction behavior where applicable
...
- [ ] TXXX Re-check dependency approvals and remove unused abstractions/global state
...
- Dependency installation tasks MUST wait for explicit user approval
...
- Keep frontend/backend ownership explicit in every task description
