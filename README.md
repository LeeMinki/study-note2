...
- 노트 기능: CRUD, 검색, 태그 필터, 단일 그룹 관리/필터, 이미지 붙여넣기, 마크다운 이미지 렌더링, 자동 임시저장
...
- 핵심 범위: 그룹 생성/조회/수정/삭제, 노트 `groupId` 할당/해제, 그룹 없음 노트 허용, 그룹 삭제 시 노트 보존 및 연결 해제, 검색/태그/그룹 AND 필터, 계정별 그룹 격리
- 현재 상태: 구현 완료
- 주요 결정:
...
- 그룹 기능: 사용자별 단일 계층 그룹, 노트당 최대 1개 그룹, 그룹 삭제 시 노트 보존 및 그룹 연결 해제
