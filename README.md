...
Study Note는 개발 및 학습 내용을 빠르게 기록하고, 검색, 태그, 그룹 필터링으로 다시 찾기 쉽게 만드는 웹 애플리케이션이다.
...
- 인증/보안: JWT 기반 로그인/회원가입, Google SSO 로그인, 계정별 노트 분리, 프로필 조회/수정, 비밀번호 변경, 명시적 CORS, JWT secret 필수화, 업로드 이미지 인증 접근 제어, 인증 rate limit
- 노트 기능: CRUD, 검색, 태그 필터, 단일 그룹 관리/필터, 그룹별 노트 미리보기, 이미지 붙여넣기, 마크다운 이미지 렌더링, 자동 임시저장
- UX: Study Note 배너 메인 이동, 파비콘, 새 노트 작성 패널 접기/펼치기, 그룹 필터 select UI, 컴포저 자체 스크롤
...
### [`013-sso-login`](specs/013-sso-login/spec.md)

- 목적: Google OAuth2 SSO 로그인 추가
- 핵심 범위: Google OAuth2 Authorization Code Flow 직접 구현 (Node.js 22 내장 fetch), 신규 SSO 계정 자동 생성, 기존 local 계정과 자동 연결, JWT URL fragment 전달, 로그인 화면 Google 버튼 추가
- 현재 상태: 구현 완료
- 주요 결정:
  - 신규 npm 패키지 없음 — Node.js 22 내장 `fetch` 활용
  - DB 스키마 변경 없음 — 기존 `provider`/`provider_id` 컬럼 활용 (012-db-migration에서 준비됨)
  - JWT를 URL fragment(`#sso-token=<jwt>`)로 전달 — 서버 로그 노출 방지
  - local↔SSO 연결 계정은 두 방법 모두 로그인 허용 (password_hash 유지)
- 문서:
  [spec](specs/013-sso-login/spec.md),
  [plan](specs/013-sso-login/plan.md),
  [tasks](specs/013-sso-login/tasks.md),
  [quickstart](specs/013-sso-login/quickstart.md)

### [`014-security-hardening`](specs/014-security-hardening/spec.md)

- 목적: SSO/DB 전환 이후 애플리케이션 보안 경계 강화
- 핵심 범위: 레거시 JSON 마이그레이션 제거, 명시적 CORS 허용 목록, JWT secret 필수화, 인증 엔드포인트 rate limit, 업로드 이미지 인증 접근 제어, PR CI `npm audit`
- 현재 상태: 구현 완료
- 주요 결정:
  - 신규 패키지 없이 in-process Map 기반 rate limit 적용
  - `/uploads` 정적 공개 서빙 제거 후 인증 라우트로 이미지 제공
  - CORS 허용 출처와 rate limit 값은 환경변수로 조정
  - high 이상 npm audit 결과를 PR 검증에 반영
- 문서:
  [spec](specs/014-security-hardening/spec.md),
  [plan](specs/014-security-hardening/plan.md),
  [tasks](specs/014-security-hardening/tasks.md),
  [research](specs/014-security-hardening/research.md),
  [data-model](specs/014-security-hardening/data-model.md),
  [quickstart](specs/014-security-hardening/quickstart.md)

### [`015-ui-polish`](specs/015-ui-polish/spec.md)

- 목적: UI 전반 Polish — 색상/간격/타이포그래피 통일, 한국어 텍스트 정리, 리치 텍스트 에디터 도입
- 핵심 범위: CSS 변수 기반 디자인 토큰(zinc/slate + violet 팔레트), 전 컴포넌트 한국어화, TipTap WYSIWYG 에디터("텍스트 편집기" / "마크다운" 이중 모드), 마크다운 이미지 붙여넣기, 텍스트 편집기 인증된 이미지 즉시 렌더링, 노트 전체화면 보기, JWT sessionStorage 전환
- 현재 상태: 구현 완료
- 주요 결정:
  - 신규 패키지: TipTap v3 (`@tiptap/react`, `@tiptap/starter-kit` 등) — 리치 에디터 전용, 사용자 승인 완료
  - JWT 저장소: `localStorage` → `sessionStorage` 전환 (탭/브라우저 종료 시 자동 삭제)
  - 기존 마크다운 노트와 신규 TipTap HTML 노트 이중 포맷 지원 (`isRichContent()` 감지)
  - `useAuthenticatedImages` 셀렉터: `src^=` → `src*=` 변경 (개발/프로덕션 URL 환경 모두 대응)
- 문서:
  [spec](specs/015-ui-polish/spec.md),
  [plan](specs/015-ui-polish/plan.md),
  [tasks](specs/015-ui-polish/tasks.md),
  [research](specs/015-ui-polish/research.md),
  [design-guide](specs/015-ui-polish/design-guide.md)

### [`016-note-groups`](specs/016-note-groups/spec.md)

- 목적: 노트를 태그와 별개의 단일 그룹으로 정리하고 그룹별로 모아본다.
- 핵심 범위: 그룹 생성/조회/수정/삭제, 노트 `groupId` 할당/해제, 그룹 없음 노트 허용, 그룹 삭제 시 노트 보존 및 연결 해제, 검색/태그/그룹 AND 필터, 계정별 그룹 격리, 그룹 관리 화면의 그룹별 노트 미리보기
- 현재 상태: 구현 완료
- 주요 결정:
  - 새 패키지 없음 — 기존 Express, SQLite, Axios, React 지역 상태로 구현
  - `groups` SQLite 테이블과 nullable `notes.group_id`를 idempotent migration으로 추가
  - 노트는 최대 1개 그룹에만 속하며, 그룹은 태그를 대체하지 않는 상위 정리 단위
  - 그룹 CRUD와 노트 그룹 필터는 모두 JWT 인증 사용자 범위에서 검증
  - 그룹 관리는 프로필 옆 `그룹 관리` 버튼으로 별도 화면에서 처리하고, 노트 작성 중에도 새 그룹을 즉시 만들 수 있음
  - 중복 그룹명 오류는 사용자 친화 한국어 문구로 표시
- 문서:
  [spec](specs/016-note-groups/spec.md),
  [plan](specs/016-note-groups/plan.md),
  [tasks](specs/016-note-groups/tasks.md),
  [research](specs/016-note-groups/research.md),
  [data-model](specs/016-note-groups/data-model.md),
  [quickstart](specs/016-note-groups/quickstart.md),
  [contract](specs/016-note-groups/contracts/groups-api-contract.md)
...
17. Google SSO 로그인 및 local↔SSO 계정 연결
18. 보안 강화: 명시적 CORS, JWT secret 필수화, 인증 rate limit, 업로드 이미지 인증 접근 제어
19. CSS 변수 기반 디자인 토큰, 전 UI 한국어화
20. TipTap WYSIWYG 에디터 ("텍스트 편집기" / "마크다운" 이중 모드)
21. 노트 전체화면 보기, JWT sessionStorage 전환
22. 노트 그룹 CRUD, 그룹 없음, 그룹/검색/태그 동시 필터
23. 그룹 관리 별도 화면, 그룹별 노트 제목/본문 미리보기, 그룹 중복명 친화 오류
24. Study Note 배너 클릭 시 메인 이동, 파비콘, 새 노트 작성 패널 접기/펼치기

## 인증 및 계정 기능

- 회원가입 필드: `name`, `displayName`, `email`, `password`, `passwordConfirm`
- 로그인 방식: 이메일/비밀번호 + JWT sessionStorage 세션 (탭/브라우저 종료 시 자동 만료)
- SSO 로그인: Google OAuth2 Authorization Code Flow, 신규 계정 자동 생성 또는 기존 계정 연결
- 프로필 기능: 현재 사용자 조회, 이름/표시 이름 수정, 현재 비밀번호 확인 기반 비밀번호 변경
- 저장 구조: SQLite (`better-sqlite3`) 기반 데이터베이스
- 그룹 기능: 사용자별 단일 계층 그룹, 노트당 최대 1개 그룹, 그룹 삭제 시 노트 보존 및 그룹 연결 해제, 그룹별 노트 미리보기
...
010 이후 016 그룹 기능까지 새 패키지 없이 Node 내장 test runner를 사용한다.
