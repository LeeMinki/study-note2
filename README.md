...
- 운영 보강:
  ...
  - k3s CoreDNS upstream을 AWS VPC resolver로 고정해 GitHub DNS 조회 실패를 방지
- 문서:
...
- 핵심 범위: 백엔드 인증/보호 라우트/JSON envelope/local JSON persistence 테스트, 프론트엔드 API URL/이미지 업로드 인증 헤더/Markdown 렌더링 테스트, 기존 `PR Checks` workflow 확장, required status checks 문서화
- 현재 상태: 구현 완료
- 주요 결정:
